...
def test_get_last_dataset_production_date(datasets, no_datasets, empty_datasets):
    ...
    assert last_production_date.start_validation_date.strftime(
        '%Y%m%dT%H%M%S') == '20150114T010000'
    assert last_production_date.end_validation_date.strftime(
        '%Y%m%dT%H%M%S') == '20150701T020000'
